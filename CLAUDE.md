...
- `db/index.ts` is the live source of truth. `ensureSchema()` is memoized per
  Worker isolate and creates or migrates `thoughts`, `books`, `book_notes`,
  `push_subscriptions`, and `notification_log` on first use. Thought rows include a long-form `notes` field linked to the
...
- `db/schema.ts` describes the same final five-table shape for Drizzle only.
...
`app/api/push/route.ts` stores one Web Push subscription per device with its
IANA time zone and local send minute, following the same rules. The Worker's
`scheduled()` export in `worker/index.ts` runs every 15 minutes (see
`triggers.crons` in `wrangler.jsonc`) and delegates to
`worker/notifications.ts`, which converts to each device's local time, claims a
row in `notification_log` before sending so a retry cannot double-send, builds
the text with the same `lib/briefing.ts` helper the Calendar `Briefing` uses,
and sends through `lib/web-push.ts`. Sending needs the `VAPID_PUBLIC_KEY`,
`VAPID_PRIVATE_KEY`, and `VAPID_SUBJECT` Worker secrets (`.dev.vars` locally,
generated by `node scripts/vapid-keys.mjs`). The local test route for the cron
handler under the installed Vite plugin is `/cdn-cgi/handler/scheduled`.

`app/api/mcp/route.ts` is the MCP route: a stateless Model Context Protocol
