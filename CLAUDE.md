...
- Auth is header-based, not a session/cookie system: `apiUserId()` in `app/api/shared.ts` reads `oai-authenticated-user-id` (set upstream by the ChatGPT connector integration — see `app/chatgpt-auth.ts`), falling back to a fixed `local-preview-user` id on `localhost`/`127.0.0.1` for local dev.
...
**Frontend**: `app/SecondBrain.tsx` is the single client-side container — it owns all state (thoughts/books/notes), does one `Promise.all` fetch on mount, and passes data + callbacks down to `CalendarTab`, `MatrixTab`, and `BooksTab`. There's no client-side data-fetching library; it's plain `fetch` + `useState`, with optimistic local updates on PATCH/DELETE that roll back by re-`load()`ing on failure. `app/page.tsx` renders `<SecondBrain />` under `dynamic = "force-dynamic"` (no static generation — the app is inherently per-user/dynamic). The current wall-clock day (`today`) is deliberately only computed client-side via `useSyncExternalStore` since the Worker runs in UTC.

**Quadrants**: the Eisenhower matrix has exactly four fixed buckets defined once in `app/quadrants.ts` (`do`, `plan`, `quick`, `later`) — this is the module to touch if quadrant labels/semantics ever change, everything else imports from it.

**`design/`** holds static HTML design mockups (`.dc.html` files, a `canvas.json`), not app code.

**`examples/d1/`** is leftover vinext-starter scaffold (empty dirs, not wired into the build) — ignore unless asked about it directly.
