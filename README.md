...
the optional scheduled date and title while showing when the note was last
edited, with the original capture date on hover; its quadrant remains fixed
while the canvas is open. Notes remain attached to that thought and are
available from both the matrix and Calendar.
...
shows when it was last edited, and deliberately has no due-date control. Existing
notes open in the same canvas.
